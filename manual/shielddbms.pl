...
sub sel($$){ # SELECT / DELETE 
	...
	my $upd = 1 if($_[0] =~ m/^upd/); # delete boolean
	my $table = $_[0];
	my $cols = $_[0];
	...
	open (TBL,"db_".$db."/tables/".$table.".tbl"); # for reading only # TODO check tables existence! WTF!
...
sub update($){ # update table userpk set username = "lol" where id = 2;
	...
	$setto =~ s/^update table ([a-z0-9_-]+) set ([a-z0-9_-]+) = ["']?([a-z0-9_-]+)["']?.*/$3/;
	my @upd = sel($_[0],"");
...
sub del($){
	...
	close TBL;
	open(TBL,">$tblfile"); #TODO check file existence
